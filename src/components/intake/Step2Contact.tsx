...
export const Step2Contact = ({ onBack, onNext }: Props) => {
  ...
  // Best-effort partial-lead save once we have email + first name + problems.
  useEffect(() => {
    if (savedRef.current) return;
    const { firstName, lastName, email, phone } = state.personalInfo;
  ...
  }, [state.personalInfo, state.problems, state.segmentation]);
  ...
  const handleContinue = async () => {
    ...
    try {
      try {
        ...
        if (result?.exists) { setStatus("exists_here"); return; }
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-serif text-2xl sm:text-3xl mb-1">How can we reach you?</h2>
        <p className="text-muted-foreground text-sm">
          We'll save your progress and send your treatment plan here.
        </p>
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <Input id="phone" type="tel" autoComplete="tel" placeholder="+1 (555) 555-5555"
      ...
      {errorMsg && <ErrorMessage variant="inline" title="Something went wrong" message={errorMsg} />}

      <div className="flex gap-3 pt-2">
        <Button variant="outline" onClick={onBack} className="flex-1" disabled={status === "loading"}>Back</Button>
        <Button className="flex-1" onClick={handleContinue} disabled={status === "loading"}>
